  ...
  if _game = 'domino' then
    ...
    from (
      select g.id, g.ticket_number as ticket, g.players_count, g.stake, g.created_at,
             ...
             (select username from public.profiles where user_id=g.player1_id) as p1,
             (select username from public.profiles where user_id=g.player2_id) as p2,
             (select username from public.profiles where user_id=g.player3_id) as p3
      from public.games g
    ...
    from (
      select g.id, g.ticket_number as ticket, g.players_count, g.stake, g.created_at,
             ...
             (select username from public.profiles where user_id=g.player2_id) as p2,
             (select username from public.profiles where user_id=g.player3_id) as p3,
             (select username from public.profiles where user_id=g.player4_id) as p4
...
begin
  if _game = 'domino' then
    select jsonb_build_object(
      ...
      'p1_name', (select username from public.profiles where user_id=g.player1_id),
      'p2_name', (select username from public.profiles where user_id=g.player2_id),
      'p3_name', (select username from public.profiles where user_id=g.player3_id),
    ...
    select jsonb_build_object(
      ...
      'p2_name', (select username from public.profiles where user_id=g.player2_id),
      'p3_name', (select username from public.profiles where user_id=g.player3_id),
      'p4_name', (select username from public.profiles where user_id=g.player4_id),
