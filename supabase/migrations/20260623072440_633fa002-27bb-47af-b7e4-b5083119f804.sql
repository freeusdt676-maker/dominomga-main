...
begin
  if _game = 'domino' then
    select jsonb_build_object(
      ...
      'p1_hand', case when g.reveal_until is not null and g.reveal_until > now() then g.player1_hand else null end,
      'p2_hand', case when g.reveal_until is not null and g.reveal_until > now() then g.player2_hand else null end,
      'p3_hand', case when g.reveal_until is not null and g.reveal_until > now() then g.player3_hand else null end
    ) into r
