DROP POLICY IF EXISTS "moves_select_participant" ON public.game_moves;
DROP POLICY IF EXISTS "moves_insert_participant" ON public.game_moves;

CREATE POLICY "moves_select_participant"
...
DECLARE
  ...
  caller_is_admin_or_backend boolean := false;
...
BEGIN
  ...
  caller_is_admin_or_backend := caller_role = 'service_role' OR public.has_role(caller, 'admin');
  ...
  IF NOT caller_is_admin_or_backend THEN
  ...
  IF NOT caller_is_admin_or_backend AND caller = _winner AND winner_score < target_score AND NOT instant_domino_win THEN
...
REVOKE ALL ON FUNCTION public.settle_game(uuid, uuid) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.settle_game(uuid, uuid) TO authenticated;
GRANT EXECUTE ON FUNCTION public.settle_game(uuid, uuid) TO service_role;